...
// serving zoom in and zoom down the 2D floorplanner
utils.changeScale = function(array, scale){
  var tmp = [];
  for (var i=0; i < array.length; i++)
...
//(x1,y1)                  (x,y)
//       ---(nx,ny)--->
utils.determineLineByVector = function(x1, y1, nx, ny, width){
  return {
  x : width*nx/Math.sqrt(nx*nx+ny*ny)+x1,
  y : width*ny/Math.sqrt(nx*nx+ny*ny)+y1}
}

// reference from http://stackoverflow.com/questions/15232356/projection-of-a-point-on-line-defined-by-2-points
utils.pointOnLine = function(line1, line2, pt) {
    var isValid = false;
    ...
    return isValid ? {x:r.x, y:r.y} : null;
}
