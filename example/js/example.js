...
/*
 * Context menu for 2D floorplan
 */

var ContextMenu2D = function(blueprint3d) {

  var scope = this;
  var selectedItem;
  this.floorplanner = blueprint3d.floorplanner;
  this.floorplan = blueprint3d.floorplanner.getFloorplan();
  var type = "";
  var mouse = {x:0, y:0};

  function init() {
    $("#context-menu-delete2D").click(function(event) {
        selectedItem.remove();
        $("#context-menu2D").hide();
    });

    scope.floorplanner.itemSelectedCallbacks.add(itemSelected);
    scope.floorplanner.itemUnselectedCallbacks.add(itemUnselected);
    scope.floorplanner.itemUnselectedCallbacks.fire(null);
    initResize();

    $("#item-input2d-delete").click(function() {
        var checked = $(this).prop('checked');
        if (checked){
          selectedItem.remove();
           $("#context-menu2D").hide();
  ...
  function itemSelected(item, objStr, mousePos) {
    selectedItem = item;
    type = objStr;
    mouse = mousePos;
    $('#item-input2d-delete').prop('checked', false);

    if (objStr == "wall"){
      $("#context-menu-name2D").text("Wall");
      $('#label-option1').text("Thickness");
      $("#input-option1").val(selectedItem.getThickness());
      ...
      if (objStr == "room"){
        ...
        $("#input-option1").val(selectedItem.getRoomType());
          ...
          else if (objStr == "window"){
              $("#context-menu-name2D").text("Window");//item.metadata.itemName);
              $('#label-option1').show();
              $('#label-option1').text("Length");
              $("#input-option1").val(selectedItem.getWidth());
            }
  ...
  function resize() {
    if (type=="wall"){
      selectedItem.thickness = $("#input-option1").val();
    }
    else if ((type=="door") || (type=="window")){
           if ($("#input-option1").val()!=null)
            hoverWindow.setWidth($("#input-option1").val());
            hoverWindow.arrangeWindow(scope.floorplan.walls);
            hoverWindow.mergeWithIntersected(scope.floorplan.getWalls());
            scope.floorplanner.callUpdateTarget();
         }
         else (type=="room")
           if (selectedItem.setWidth($("#input-option1").val())!=""){
             selectedItem.setRoomType(selectedItem.setWidth($("#input-option1").val()));
             selectedItem.labelPos = {x:mouse.x, y:mouse.y};
           }
  }

  function initResize() {
    $("#item-height2D").change(resize);
    $("#item-width2D").change(resize);
    $("#item-depth2D").change(resize);
  }
...
var ViewerFloorplanner = function(blueprint3d) {
  ...
  var remove = '#delete';
  var draw = '#draw';
  var draw_room = '#draw-room';
  var draw_door = "#draw-door";
  var draw_window = "#draw-window";
  var clear = "#clear";
  var ruler = "#ruler";
  ...
  function init() {
    ...
    // mode buttons
    scope.floorplanner.modeResetCallbacks.add(function(mode, unit) {
      $(draw).removeClass(activeStlye);
      $(draw_room).removeClass(activeStlye);
      $(remove).removeClass(activeStlye);
      $(move).removeClass(activeStlye);
      $(draw_door).removeClass(activeStlye);
      $(draw_window).removeClass(activeStlye);
      $(ruler).removeClass(activeStlye);
      $(clear).removeClass(activeStlye);
      $(roomtype).removeClass(activeStlye);
      $(meter).removeClass(activeStlye);
      $(feet).removeClass(activeStlye);
      ...
      if (mode == scope.floorplanner.modes.MOVE) {
        ...
        } else if (mode == scope.floorplanner.modes.DRAW_ROOM) {
          $(draw_room).addClass(activeStlye);
          } else if (mode == scope.floorplanner.modes.DRAW_DOOR) {
            $(draw_door).addClass(activeStlye);
            } else if (mode == scope.floorplanner.modes.DRAW_WINDOW) {
            $(draw_window).addClass(activeStlye);
             } else if (mode == scope.floorplanner.modes.RULER) {
              $(ruler).addClass(activeStlye);
              } else if (mode == scope.floorplanner.modes.CLEAR) {
               $(clear).addClass(activeStlye);
                } else if (mode == scope.floorplanner.modes.ROOMTYPE) {
                 $(roomtype).addClass(activeStlye);
                 }

        if (unit == scope.floorplanner.units.METER) {
            $(meter).addClass(activeStlye);
            } else if (unit == scope.floorplanner.units.FEET) {
              $(feet).addClass(activeStlye);
               }

/*      if (mode == scope.floorplanner.modes.DRAW || mode == scope.floorplanner.modes.DRAW_ROOM || mode == scope.floorplanner.modes.DRAW_DOOR) {
    ...
    $(draw_window).click(function(){
      scope.floorplanner.setMode(scope.floorplanner.modes.DRAW_WINDOW);
    });

    $(clear).click(function(){
      scope.floorplanner.setMode(scope.floorplanner.modes.CLEAR);
      scope.floorplanner.clearAll();
    });

    $(ruler).click(function(){
      scope.floorplanner.setMode(scope.floorplanner.modes.RULER);
    });

    $(draw_door).click(function(){
      scope.floorplanner.setMode(scope.floorplanner.modes.DRAW_DOOR);
    });

    $(roomtype).click(function(){
      var direction = prompt("Type the direction: ", compass);
		  if (direction && direction!=""){
			compass = direction;
			scope.floorplanner.setCompass(compass);
			}
    });

    $(meter).click(function(){
      scope.floorplanner.setUnit(scope.floorplanner.units.METER);
    });

    $(feet).click(function(){
      scope.floorplanner.setUnit(scope.floorplanner.units.FEET);
    });

    $(submit).click(function(){
      scope.floorplanner.setThickness(document.getElementById("#edit-thickness").value);
      $("#submitdiv").hide();
    });
  }
...
$(document).ready(function() {
  ...
  var contextMenu2D = new ContextMenu2D(blueprint3d);
